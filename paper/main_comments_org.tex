...
%    }
    ...
    \IEEEauthorblockN{
        Omitted for blind review \IEEEauthorrefmark{1},
...
\begin{abstract}
\textcolor{green}{scrivete le sezioni in latex separati e usante input per inserirle nel main, stesso per figure , tabelle e grafici tutti in file separati e inseriti poi nel main, create un file tex con i packages a con i nuovi comandi se servono }
...
compiler and evaluate it against randomized-restart and WISQ
baselines.
\todo{Insert one or two results once tables are filled (e.g.\ mean
routing-step reduction vs.\ baseline; compilation time).}
\end{abstract}
...
\textcolor{green}{intro si struttura: fault tolerant QC, problema,  come lo risolviamo, related (alemno 5 o 6 lavori con mini descrizione di cosa fanno e dove noi ci differenziamo),, contributi- non deve essere super tecnico qui ma dare un idea al lettore}
...
\textcolor{green}{i contributi vanno quantificati e.g. andiamo meglio di questi di un tot percento..}
\begin{itemize}
    ...
    against a randomized-restart baseline and the WISQ compiler
...
\textcolor{green}{Qui massimo 1-2 sezioni: il flow deve essere cos è la FTQC, QEC, come si formlaizza su un grafo,  cos è lattice surgery,maximum edges disjoint problem complessita etc, magari con delle figure un esempio di circuito -grafo -LS puo bastare }
...
\label{sec:approach}
\textcolor{green}{qui va un po rivisto ci sono termini non definiti e formule pendenti, io metterei idea del influence field, mapping coarse e fine , e una sezione che descrive  il vostro loop con anche il routing che non viene proprio citato, pseudo codice ok se vi è utile per il testo, in genrale questa è la parte core del paper almeno 2-3 pg}
The core idea is to convert the objectives of fault-tolerant
...
\vspace{2pt}
\subsubsection{Magic-state fields}
...
\subsubsection{CNOT (partner) fields}
attractive Gaussians centred on the already-mapped tiles of
...
w =\!
\begin{cases}
\textsc{magic-high}, & \hspace{-30pt} |t(q)-\bar t|>\sigma_t,\\[2pt]
\mathrm{lerp}\big(\textsc{magic-low},\textsc{magic-high},\tfrac{|t(q)-\bar t|}{\sigma_t}\big), & \hspace{15pt} \text{else},
\end{cases}
\end{equation*}
...
routes the result with a lattice-surgery router. The \emph{packing}
...
\subsubsection{Baselines and metrics} We compare our (fine) Gaussian method
against two baselines:
(i) \emph{random}, a fast placement that scatters qubits over a sparse
sublattice, enforcing $3\times3$ spacing to make every mapping trivially
routable and using the same router as Gaussian; and 
(ii) the external WISQ compiler~\cite{wisq}. The random baseline is
the natural speed reference: each restart is so cheap that we run it for \todo{N}
repetitions and keep the best result, whereas the Gaussian placement
is deterministic and runs once. The primary metrics are the number of
routing steps and compilation time; our implementation also provides average parallelism and average 
non-routed-gate percentage across layers.

\subsection{Results}

\paragraph{Routing quality}
Table~\ref{tab:results} reports routing steps and compilation time across the
benchmark suite for the Gaussian method and the randomized-restart and WISQ
baselines.
...
The field weights of Section~\ref{sec:fields} were tuned per regime---the four
combinations of weighting variant (coarse or fine) and safe-passage strategy
(cube or connectivity)---by benchmark sweeps; the full values are provided in the
project repository~\cite{ftqc}.
...
\todo{2--4 sentences once numbers are in:
(i) where the field method beats / loses to WISQ and why; (ii) the value of
...
% beverland2022surface} establish the routing primitives and cost metrics we optimise. 
% WISQ~\cite{wisq} is a recent end-to-end fault-tolerant compiler that we use as an external
% baseline. Relative to solver- and search-heavy schedulers, our contribution is
% a cheap, interpretable \emph{placement} heuristic that produces routable mappings 
% for a simple downstream router.
...
\textcolor{green}{in generale evitate troppe sotto sezioni, elenchi puntati a, b ,c .. il testo deve essere fluido non una serie di statements, attenti a formule  costanti e simboli non spiegati, quando fate una nuova sezione due righe di cosa c e dentro prima di partire con le sotto sez, tenetevi sulle 6pg + 1 pg per le ref}
...
partner fields with repulsive fields, then places each qubit at the
safe maximum of the result. This folds $T$-gate
locality, interaction clustering, and corridor preservation into a single fast
scoring pass, while safe-passage keeps every mapping routable. We
implement this heuristic in an open-source OpenQASM-to-lattice-surgery compiler and evaluate
it against a randomized-restart baseline and the WISQ compiler.
\todo{State the result once tables are filled.}
