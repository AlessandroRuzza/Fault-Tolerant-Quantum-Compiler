...
\begin{table}[t]
    \centering
    \caption{Routing steps and compile time per benchmark. ``Gauss'' is the
    (fine) Gaussian placement, ``Rand'' the best of \todo{???} randomized baselines. 
    $L_{\min}$ is the circuit's depth in dependency-layered form (its layer count)---
    a lower bound on routing steps, since data-dependent layers cannot share a step.
    \todo{Fill from \texttt{best\_params\_benchmark\_runs.csv} and
    \texttt{wisq\_docker.csv}.}}
    \label{tab:results}
    ...
    \begin{tabular}{l r r rrr rrr}
        ...
        \textbf{Circuit} & \textbf{$|\Q_L|$} & \textbf{$L_{\min}$}
            ...
            & \multicolumn{3}{c}{\textbf{Compile time (s)}} \\
...
\begin{figure}[t]
    \centering
    \figslot{1.6in}
    \caption{Per-circuit routing step ratios: Gaussian field placement / WISQ~\cite{wisq}. 
    Points below the diagonal indicate fewer routing steps for our method. 
    \todo{Generate from merged benchmark CSVs.}}
    \label{fig:scatter}
    \vspace{-0.5cm}
\end{figure}
...
(iii) scalability --- circuits on which WISQ times out (too large search space) 
but field placement finishes fast.}
