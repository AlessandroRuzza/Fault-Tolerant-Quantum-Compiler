...
lattice, then place each qubit at the field's maximum that respects \emph{safe passage}.
...
w^{\mathrm{mag}} \;=\;
\begin{cases}
\textsc{magic-high}, & t(q)>c_{\max}(q)\ \text{ or }\ |t(q)-\bar t|>\sigma_t,\\[2pt]
0, & \text{otherwise,}
\end{cases}
\end{equation*}
the field being attractive except in the partner-driven, $T$-light case
($t(q)\le c_{\max}(q)$ and $t(q)<\bar t-\sigma_t$), where it is repulsive. In the
partner-driven regime, coarse also adds an attractive CNOT field at every mapped
partner $p$ with above-average interaction ($c(q,p)\ge\lceil\bar c\rceil$), of weight
\begin{equation*}
w^{\mathrm{cnot}}_{p} \;=\;
\begin{cases}
...
\end{cases}
\end{equation*}
tripling the pull toward the single dominant partner $p^{\star}=\arg\max_{p'}c(q,p')$.
