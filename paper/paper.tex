...
of lattice that must stay open so interacting patches, or a patch and the magic
state it needs, can be joined on demand. We present a \emph{Gaussian
...
every layout routable. We implement the method in an open-source C++20 OpenQASM
...
maps and routes on smaller lattices. \emph{Cube} instead applies a
stricter geometric rule---it forbids any tile within the $3\times3$ neighbourhood
of an occupied patch. This requires a larger grid
...
\emph{cube} as the default and \emph{connectivity} as the option for when lattice
area is constrained.
...
\emph{Coarse} makes discrete decisions. If a qubit's $T$-count $t(q)$ outweighs
its largest CNOT count with any single partner, the qubit is treated as
$T$-bound and its magic fields are made strongly attractive (weight
\textsc{magic-high}). Otherwise placement is partner-driven: attractive CNOT
fields (weight \textsc{cnot-high}) are added at its mapped partners, and the
magic fields are turned attractive only when $t(q)>\bar t+\sigma_t$, repulsive
when $t(q)<\bar t-\sigma_t$, and left off in between.
...
scoring pass, while the safe-passage filter keeps every layout routable. We
