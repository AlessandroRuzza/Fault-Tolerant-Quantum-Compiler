...
the first that keeps the mapping in safe passage
...
The resulting mapping is handed to the router, which turns it into a routing
$\Rout$ (Definition~\ref{def:routing}). Our \emph{packing} router opens as many
...
strategy that reserves a lane to the nearest free magic-state tile; a simpler
\emph{naive} shortest-path router is also available. Because the mapping already
...
by packing each step densely. The full router is described in the
artifact~\cite{ftqc}.
