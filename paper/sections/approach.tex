...
In contrast to compiler flows that jointly optimize placement, routing and
scheduling through global search or solver-based
procedures~\cite{wisq,zhu2024ecmas}, we treat initial placement as a fast
geometric approximation problem. The dominant fault-tolerant pressures can be
estimated \emph{before} routing begins: a qubit with high non-Clifford demand
should sit near magic-state supply, strongly interacting qubits should sit near
one another, and already-placed patches should repel later placements to preserve
routing capacity. We encode these pressures as a single scalar field over the
lattice and commit each qubit to the highest-scoring tile that is still provably
safe for downstream routing. The field acts as an interpretable proxy for routing
difficulty: attractive terms estimate future communication demand, while repulsive
terms reserve free space for that demand to materialize without deadlock.

The rest of this section specifies the method. We first define the
...
A single \emph{influence field} is an isotropic 2D Gaussian centered at a tile
$(\mu_x,\mu_y)$,
\begin{equation}
...
$\max\!\big(w-g(x,y),\,0\big)$, which is near zero at the center and rises to $w$
...
evaluated only within a bounded window around its center, where its contribution
...
clearing those neighborhoods for the qubits that need them. The
...
Gaussian at the grid center that keeps the overall mapping compact; in the regime
where magic states sit on the lattice boundary we subtract a tuned constant from every border tile, penalizing boundary placements that obstruct magic state tiles.
...
neighborhood breadth-first, heaviest partner first, so every qubit finds its
...
Algorithm~\ref{alg:main} summarizes the loop and Figure~\ref{fig:frames} shows the potential-field variation as
the mapping grows. The loop is intentionally a deterministic, low-overhead
front-end mapper rather than a globally optimal scheduler: in a regime where exact
optimization is $\NP$-hard~\cite{herr2017nphard} and the best compilation strategy
depends on workload properties such as circuit density, logical-qubit count and
$T$ fraction~\cite{leblond2025comparison}, we deliberately prioritize
interpretability, compile-time efficiency and routability-preserving geometry over
global optimality.
...
vertex-disjoint corridors per step $P_i$ as it can, prioritizing gates that are
