...
Only circuit x graph-dimensions where `gaussian`, `homogeneous`, and `magic-aware` all appear in the runs. Best configuration is chosen among successful runs with routing steps available.
...
Only circuit x graph-dimensions where `gaussian`, `homogeneous`, and `magic-aware` all appear among runs with `exit_code = 0`. Best configuration is chosen only from those `exit_code = 0` runs.
