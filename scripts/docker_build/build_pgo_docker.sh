...
# use inside a Docker build (see docker/Dockerfile.PGO). Same two-phase flow as
# build_pgo.sh, but:
...
#
# Usage:
#   scripts/docker_build/build_pgo_docker.sh [BENCH] [MARCH]
...
REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"  # scripts/docker_build/ -> root
cd "$REPO_ROOT"
