...
For each circuit in mapping_only_benchmark.json:
  ...
  2. Compute the starting dim using compute_upper_dimensions (via a single
     --x 0 call to the binary so we don't have to replicate the formula here).
...
This script drives the binary in MAPPING_ONLY mode (env FTQC_MAPPING_ONLY=1):
mapping success at a given grid size is the only thing the min/max-dimension
question depends on, so the binary stops right after mapping and skips the much
slower (and, for this purpose, irrelevant) layering/routing stage.

Dimension sentinels passed via --x/--y (must match the binary's one_execution):
  >0 -> use the grid verbatim,  0 -> compute_upper_dimensions (worst-case upper
  bound),  <0 -> heuristic compute_dimensions + dimension_offset.
"""
...
CLI_MAP = {
    ...
    "bfs_density_threshold": "--bfs-density-threshold",
...
def run_one(cfg: dict, x: int, y: int, timeout: int) -> tuple[int, str]:
    ...
    FTQC_MAPPING_ONLY=1 stops the binary right after mapping (skips
    layering/routing): a complete mapping is exactly the feasibility signal
    this analysis needs, and skipping routing makes the search far faster.
    """
    cmd = build_cmd(cfg, x, y)
    env = dict(os.environ)
    env["FTQC_BENCH_WORKER"] = "1"
    env["FTQC_MAPPING_ONLY"] = "1"
...
def get_upper_dim(cfg: dict, timeout: int) -> tuple[Optional[int], bool]:
    """Single binary call with x=0 (compute_upper_dimensions). Returns
    (upper_dim, no_routable_gates_flag)."""
    exit_code, stdout = run_one(cfg, 0, 0, timeout)
    if exit_code != 0:
        return None, False
...
def main() -> int:
    ...
    for circuit, configs in by_circuit.items():
        ...
        # Step 1: probe with one config at x=0 to get upper_dim and detect
