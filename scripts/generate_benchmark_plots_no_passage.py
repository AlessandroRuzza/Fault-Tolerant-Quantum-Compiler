...
REQUESTED_SAFE_PASSAGES = set()
EXCLUDED_SAFE_PASSAGES = {"passage", "passage_no_subgraphs", "cube"}
...
def default_output_dir_for_single_csv(csv_path, distinct):
    suffix = "_merge_plots_no_passage_no_cube" if distinct else "_plots_no_passage_no_cube"
    return os.path.join(
...
def default_output_dir_for_glob(distinct):
    if distinct:
        return os.path.join(DEFAULT_RESULTS_DIR, "merge_plots_no_passage_no_cube")
    return os.path.join(DEFAULT_RESULTS_DIR, "plots_no_passage_no_cube")
    ...
    for path in files:
        with open(path, newline="", encoding="utf-8") as f:
            ...
            for row_index, raw in enumerate(reader):
                ...
                if normalize_text(row.get("safe_passage_strategy")) in EXCLUDED_SAFE_PASSAGES:
                    continue
                rows.append(row)

    # Keep only circuits that appear in every accepted CSV file.
    if len(accepted_files) > 1:
        circuits_per_file = defaultdict(set)
        for row in rows:
            circuit_name = os.path.basename(row.get("circuit", ""))
            circuits_per_file[row["source_csv"]].add(circuit_name)
        common_circuits = set.intersection(*circuits_per_file.values()) if circuits_per_file else set()
        rows = [row for row in rows if os.path.basename(row.get("circuit", "")) in common_circuits]
