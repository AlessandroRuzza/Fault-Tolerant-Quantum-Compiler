#!/usr/bin/env python3
"""Throwaway: Fig.5 (steps/gate scatter, ours vs WISQ) but the dots are joined
into a line, sorted by the x value (WISQ steps/gate). connectivity+packing CSV."""
...
print(f"n={n} below(win)={wins} on={ties} above(loss)={losses}")

fig, ax = plt.subplots(figsize=(3.4, 3.2))
ax.plot(xs, ys, color="#1565C0", linewidth=1.0, alpha=0.9, zorder=2,
        marker="o", markersize=3, markerfacecolor="#1565C0",
...
fig.tight_layout()
fig.savefig(OUT / "7_scatter_per_gate_joined.png", dpi=170)
plt.close(fig)
print("saved", OUT / "7_scatter_per_gate_joined.png")
