...
Reads the canonical 258-circuit sweep (data/results/cube_summary_all_wisq_dinuovo.csv)
and plots a log-log scatter of WISQ routing steps (x) against our routing steps (y),
one point per circuit where WISQ completed. Points below the y=x diagonal are
circuits where we use fewer routing steps than WISQ.
...
INPUT_CSV = REPO_ROOT / "data" / "results" / "cube_summary_all_wisq_dinuovo.csv"
...
def main() -> int:
    ...
    ax.set_ylabel("Gaussian routing steps")
    ax.set_aspect("equal")
